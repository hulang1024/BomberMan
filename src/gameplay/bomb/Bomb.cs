...
{
    public static PackedScene Scene = GD.Load<PackedScene>("res://src/gameplay/bomb/Bomb.tscn");

    public Action OnDestory;

    public const int MaxPower = 14;

    public Character PlaceBy;
    ...
    {        
        var centerCellPosition = World.Map.WorldToMap(GlobalPosition);
        var centerFlame = FlameSegment.Create(this, FlameSegment.Kind.Center);
        World.Map.ReplaceTile(centerCellPosition, centerFlame);
        ...
        for (int power = 0; power < Power; power++)
        {
            for (int dir = 0; dir < 4; dir++)
            {
                ...
                switch (World.Map.GetTileId(position))
                {
                    case MapTile.Brick:
                        if (!stopPowerDirs.ContainsKey(dir))
                        {
                            ...
                            World.Map.ReplaceTile(position, brokenBrick);
                        }
                    ...
                    case MapTile.Empty:
                        ...
                        else
                        {
                            ...
                            World.Map.ReplaceTile(position, flame);
                        }
                        break;
                }
            }
